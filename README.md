...
## สรุปการปรับ UX ล่าสุด — Popup แทนการเปลี่ยนหน้า + จัดโครงสร้างโมดูล C

**Popup (BottomSheetModal) แทนการสลับเนื้อหาในหน้าเดิม** — ใช้กับทุกโมดูลที่ต้องสแกน/กรอกรหัส
สินค้า (A, B, C, D1, E): เดิมสแกนแล้วทั้งหน้าเปลี่ยนไปเป็นฟอร์มกรอกจำนวน (กล้องถูกปิด) ตอนนี้กล้อง/
ลิสต์เบื้องหลังยังแสดงอยู่ตลอด ป๊อปอัพแค่ลอยทับด้านบนแล้วหายไปเมื่อทำรายการเสร็จ

- **เร็วขึ้นจริง ไม่ใช่แค่ความรู้สึก**: เพิ่ม prop `paused` ใหม่ให้ `QRScanner.jsx` (คนละอย่างกับ
  `disabled` ที่ปิดกล้องจริง) — ตอนป๊อปอัพเปิดอยู่ กล้องยังทำงานต่อเนื่องเบื้องหลังตามปกติ แค่ไม่รับ
  ผลตรวจจับใหม่ชั่วคราว พอปิดป๊อปอัพพร้อมสแกนต่อทันทีโดยไม่ต้องรอกล้องเปิดใหม่ (ซึ่งมี latency)
- ตรวจสอบแล้วว่า dialog อื่นๆ (เช่น "เจอ SKU ซ้ำ") ยังคงลอยทับป๊อปอัพนี้ได้ถูกต้อง (z-index
  `BottomSheetModal` = 2400, `ConfirmDialog` = 2500)
- ไล่ตรวจ logic เดิมทุกไฟล์ที่แก้ (state, การเรียก service, draft, GPS ฯลฯ) ไม่มีจุดไหนเปลี่ยน
  พฤติกรรม เปลี่ยนแค่ตัว JSX ที่ห่อ (wrapper) เท่านั้น

**จัดโครงสร้างหน้า "บันทึก/ตีคืนสินค้า" (โมดูล C) ใหม่** ตามลำดับความสำคัญ:
1. รายการที่ยังไม่ปิดรอบ (พร้อมปุ่ม "ยกเลิกใบนี้" ต้องกรอกรหัสผ่านสาขายืนยันก่อนเสมอ)
2. ประวัติใบที่ส่งแล้ว
3. เสีย (คัดออก/ทำลายหน้าร้าน) — สีแดง/ส้ม
4. แถม/เคลม/อื่นๆ — สีเขียว/ฟ้า/ม่วง (เล็กกว่า เห็นชัดว่าสำคัญรองลงมา)

แต่ละหมวดมีสีประจำตัว (`tone` ใน `returnCategories.js`) แสดงเป็นแถบสีทั้งตอนเลือกหมวดและตอนดู
รายการ กันพนักงานกรอกผิดหมวด

**แก้บั๊กวันที่แสดงผลเพี้ยน**: locale `th-TH` บางเบราว์เซอร์ auto ขึ้นปีพุทธศักราชในบางฟังก์ชัน
(ไม่ใช่ทุกจุด) ทำให้เอกสารเดียวกันมีปีไม่ตรงกัน — บังคับ `calendar: 'gregory'` ทุกจุดที่ format
วันที่แบบ locale แล้ว

**เพิ่ม `WorkDateTimeBar`** แสดงวันที่/เวลาปัจจุบันกำกับหัวงานในหน้ารับเข้า/ตีคืน
